ALTER PROCEDURE dbo.spSendPeymankarAllowSMS
AS
BEGIN
	...
	DECLARE @lMPFeederName AS NVARCHAR(50)
    ...
    LEFT JOIN Tbl_MPFeeder MPF ON tTR.MPFeederId = MPF.MPFeederId
	WHERE 
	...
	FETCH NEXT
	FROM db_cursor
	INTO @lReqAreaID, @lRequestNumber, @lBazdidID, @lAllowNumber, @lPeymankar, @lPeymankarMobile , @lMPFeederName
	...
	BEGIN
		...
		SET @lSMSBuff = Replace(@lSMSBuff, 'Paymankar', ISNULL(@lPeymankar, ''))
		SET @lSMSBuff = Replace(@lSMSBuff, 'MPFeederName', ISNULL(@lMPFeederName, '؟'))
