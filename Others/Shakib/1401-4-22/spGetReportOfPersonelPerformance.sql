...
BEGIN
	...
	DECLARE @MaxTotalRegistration AS INT
		...
		,@MaxDelaysInRegistration AS INT
		,@MaxDelaysInRegistration3 AS INT
	 ...
	 */
	UPDATE #tempEachRow
	SET CoefficientOfRegistration = ISNULL((1.00*TotalRegistration - 2) / NULLIF(@MaxTotalRegistration - 2,0),0)
	...
	SET DelaysInRegistration2 = 
			ISNULL((1.00 * DelaysInRegistration - 1) / (NULLIF(@MaxDelaysInRegistration,1) - 1),0)
		,DelaysInRegistration3 = 
			ISNULL(1 / NULLIF(((ISNULL(1.00* DelaysInRegistration,1) - 1) / (NULLIF(@MaxDelaysInRegistration,1) - 1)),0),0)
	FROM #tempEachRow
 ...
 تاخیر در ثبت ها */
	...
	SET OperatorPerformanceInlateDelaysRegistration =
										 ISNULL((1.00*DelaysInRegistration3 - 1) / NULLIF(@MaxDelaysInRegistration3 - 1,0),0)
	FROM #tempEachRow
	...
	UPDATE #tempEachRow
	SET OvertimeBasedOnSystemPerformance = ISNULL(((1.00 * Overtime_30_50_20 * 14) / NULLIF(@AVGOvertime_30_50_20,0)),0)
		,FinalOvertime_withAverage20 = ISNULL(((1.00 * Overtime_30_50_20 * 14) / NULLIF(@AVGOvertime_30_50_20,0)),0) + 6
	FROM #tempEachRow
	...
	SELECT @MaxTotalRegistration MaxTotalRegistration
		...
		,@MaxDelaysInRegistration3 MaxDelaysInRegistration3
