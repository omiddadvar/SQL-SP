...
BEGIN
	...
	DECLARE crMakeList CURSOR FOR
		SELECT 
			ConnectType = CASE WHEN IsManoeuvre = 1 THEN 'وصل با مانور:' ELSE 'وصل موفق: ' END,
...
AS
	...
	SELECT 
		...
		@MPPostInfo = 'پست فوق توزيع ' + Tbl_MPPost.MPPostName + 'CRLF', 
		@MPFeederName = Tbl_MPFeeder.MPFeederName, 
		@MPFeederInfo = 'فيدر فشار متوسط ' + Tbl_MPFeeder.MPFeederName + 'CRLF', 
		@LPPostName = Tbl_LPPost.LPPostName, 
		@LPPostInfo = 'پست توزيع ' + Tbl_LPPost.LPPostName + 'CRLF', 
		@FeederPartInfo = 'تکه فيدر ' + Tbl_FeederPart.FeederPart + 'CRLF', 
		@LPFeederInfo = 'فيدر فشار ضعيف ' + Tbl_LPFeeder.LPFeederName + 'CRLF',
		...
		@WarmLineMode = CASE WHEN TblTamirRequest.IsWarmLine = 1 THEN 'خط گرم' ELSE 'خط سرد' END,
	...
	BEGIN
		SET @MPFeedersInfo = 'فيدرهاي فشار متوسط ' + LEFT(@MPFeedersName, (LEN(@MPFeedersName) - 1)) + 'CRLF'
		SET @MPFeederInfo = ''
	END
	...
	BEGIN
		SET @Status = N'تاييد گرديد'
		if @IsWarmLine = 1
			SET @WarmLineAdvance = 'بصورت خط گرم'
		if @IsWarmLine = 0
			SET @WarmLineAdvance = 'بصورت خط سرد'
	END
	if @TamirRequestStateId = 8 
		SET @Status = N'تاييد نگرديد'
		...
		BEGIN
			...
			BEGIN
				...
				IF NOT @Description IS NULL
					SET @Reason = @Reason + ' و نوع اشکال ' + @Description
				ELSE IF NOT @FogheToziType IS NULL 
					SET @Reason = @FogheToziType

				IF @IsFT = 1
					SET @ReqType = N'فوق توزيع'
				ELSE IF @IsMP = 1
					SET @ReqType = N'فشار متوسط'
				ELSE IF @IsLight = 1
					SET @ReqType = N'روشنايي معابر'
				ELSE IF @IsLP = 1
					SET @ReqType = N'فشار ضعيف'
				ELSE
					SET @ReqType = N'جديد'
				...
				BEGIN
					SET @PostFeeder = N' فيدر فشار ضعيف ' + @LPFeederName
					SET @PostFeederLPF = N' فيدر فشار ضعيف ' + @LPFeederName
				END
				IF NOT @LPPostName IS NULL
				BEGIN
					SET @PostFeeder = N' پست توزيع ' + @LPPostName + @PostFeeder
					SET @PostFeederLPP = N' پست توزيع ' + @LPPostName
				END
				IF NOT @MPFeederName IS NULL
				BEGIN
					SET @PostFeeder = N' فيدر فشار متوسط ' + @MPFeederName + @PostFeeder
					SET @PostFeederMPF = N' فيدر فشار متوسط '
				END
				IF NOT @MPPostName IS NULL
				BEGIN
					SET @PostFeeder = N' پست فوق توزيع ' + @MPPostName + @PostFeeder
					SET @PostFeederMPP = N' پست فوق توزيع '
				END
...
/* ------------------- پيامک ارسال هشدار براي خاموشيهاي بابرنامه اي که درصدي از زمان قطع آن گذشته باشد -------------------- */
	...
	DECLARE RequestList CURSOR FOR
		...
		BEGIN
			...
			BEGIN
				...
				IF @lMPRequestId > -1
					SET @lTamirNetworkType = N'فشار متوسط'
				ELSE IF @lLPRequestId > -1
					SET @lTamirNetworkType = N'فشار ضعيف'
				ELSE IF @lFogheToziDisconnectId > -1
					SET @lTamirNetworkType = N'فوق توزيع'
					
				SELECT 
					@lTamirRequestId = TblTamirRequest.TamirRequestId,
					@lRequestBy = CASE WHEN TblTamirRequest.IsRequestByPeymankar = 1 THEN N'پيمانکار' WHEN TblTamirRequest.TamirNetworkTypeId = 1 THEN N'فوق توزيع' ELSE N'شرکت توزيع' END, 
...
/* ------------------- ارسال پيامک يادآوري تعيين وضعيت خاموشيهاي درانتظار تاييد -------------------- */
...
/* ----------------------------  ارسال پيامک طولاني تر شدن زمان خاموشي از زمان پيش بيني شده بابرنامه -------------------- */
	...
	DECLARE crManagerDC CURSOR read_only FOR
		...
		BEGIN
			...
			BEGIN
				...
				-- خاموشي فشار ضعيف با برنامه کلي
				...
				-- خاموشي فشار ضعيف با برنامه تکي
				...
				-- خاموشي فشار ضعيف بي برنامه کلي
				...
				-- خاموشي فشار ضعيف بي برنامه تکي
				...
				-- خاموشي فشار متوسط با برنامه کلي
				...
				-- خاموشي فشار متوسط با برنامه تکي
				...
				-- خاموشي فشار متوسط بي برنامه کلي
				...
				-- خاموشي فشار متوسط بي برنامه تکي
				...
				-- خاموشي جديد بي برنامه فيدر فشار متوسط <omid>
  ...
  -------------</omid>
	...
	DECLARE crInform CURSOR FOR
		...
		BEGIN
            ...
            SET @SMS = Replace( @SMS,'SMSSubject', ISNULL(@SMSSubject,N'تعميرات شبکه'))---------omid
...
AS
	...
	SELECT TOP 1 
		...
		@OwnerShip = ISNULL(OwnerShip,N'عمومي'),
...
AS
	...
	SELECT TOP 1 
		...
		@CurrentValueConnectInfo = 'با بار ' + ISNULL(CAST(CurrentValueConnect AS varchar),'؟') + ' آمپر '
	FROM 
		TblMPRequest 
	WHERE  
	...
	DECLARE crSerghatList CURSOR FOR
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					IF @RecordType = 'MPLP'
						SET @ReqType = N'فشار متوسط و فشار ضعيف'
					...
					ELSE IF @RecordType = 'Erja'
						SET @ReqType = N'ارجاع شده به اين واحد'
					...
					SET @SMS = Replace( @SMS,'Hello', N'با سلام
' )
					SET @SMS = Replace( @SMS,'CRLF', nchar(13) )
...
/* ------------------- پيامک ارسال هشدار به پيمانکاران براي خاموشيهاي بابرنامه اي که درصدي از زمان قطع آن گذشته باشد -------------------- */
	...
	DECLARE MultistepConnectionList CURSOR FOR
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					SELECT 
						@ConnectType = CASE WHEN IsManoeuvre = 1 THEN 'به وسيله مانور' ELSE 'با موفقيت' END,
...
/* ------------------- ارسال پيامک تاييد يا عدم تاييد خاموشي هاي بابرنامه به پيمانکاران -------------------- */
...
AS
	...
	BEGIN TRY
		...
		IF (@lCounter % 2) = 0 
		BEGIN
			SET @lOperation = @lOperation + N'(توليد پيام خاموشي‌ها براي مشترکين)(توليد فکس خاموشي ها براي مشترکين)(توليد ايميل خاموشي ها براي مشترکين)(توليد تماس تلفني خاموشي ها براي مشترکين)(توليد پيام بعد از وصل خاموشي براي مشترکين)(توليد پيام در صورت عدم اعزام اکيپ براي خاموشي اعلام شده توسط مشترک)'
